...
#define HEXDIGIT(c) (isalpha(c) ? tolower(c) - 'a' + 10 : (c) - '0')
#define HEX4_TO_UNSIGNED(s)                                                    \
  ((HEXDIGIT(s[0]) << 12) | (HEXDIGIT(s[1]) << 8) | (HEXDIGIT(s[2]) << 4) |    \
   (HEXDIGIT(s[3]) << 0))
static const char *lept_parse_hex4(const char *p, unsigned *u) {
  if (!(isxdigit(p[0]) && isxdigit(p[1]) && isxdigit(p[2]) && isxdigit(p[3]))) {
    return NULL;
  }
  *u = HEX4_TO_UNSIGNED(p);
  return p + 4;
}
...
#define CODEPOINT(H, L) (0x10000 + ((H) - 0xD800) * 0x400 + (L - 0xDC00))
